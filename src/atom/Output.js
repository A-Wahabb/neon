                ...
                <div className="container text-center h-100" style={oldProps.BackGroundWall} >
                    <Textfit className='viewContainer mx-auto mt-3' mode="multi">
                        {oldProps.UserInput ? oldProps.UserInput.split("\n").map((l, i) =>
                            <p className={`bg-transparent mb- responsiveText ${oldProps.Color} ${oldProps.ShowShadow ? 'text-white active' : 'text'}`} style={oldProps.mystyle} key={i}>{l}</p>
                        ) :
                            <p className={`bg-transparent mb- responsiveText ${oldProps.Color} ${oldProps.ShowShadow ? 'text-white active' : 'text'}`} style={oldProps.mystyle} >Type Here</p>}
                    </Textfit>
